...
/* 
value从keyboard可以传入，NoteInBucket也可以传入
active从哪来的，从bucket那边可以传入active，但是keyboard这边不会传入
*/
const Note = ({ value, active }) => {

  // 这个是播放时的黄颜色，如果不是播放的时候下面会重新赋值颜色
  let color = '#F4A53F';

  // 当note是keyboard这边的时候active是undefined，就会进入下面的if判断
  if (!active)
